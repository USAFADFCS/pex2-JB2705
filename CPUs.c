...
void* NPPcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {;
            ...
            // NPP selection rule is based on priority. We will set p to whatever the highest priority index found in the ready queue is
            int priorityPos = qPriority(&(svars->readyQ));
            p = qRemove(&(svars->readyQ), priorityPos);
...
// ============================================================
// RR — Round Robin (WIP->Copied code from FIFO)
...
void* RRcpu(void* param) {
    ...
    //set variables for tracking the quantums
    ...
    while (1) {
        ...
        //preemptive-chech the quantum
        if (p!=NULL && currQuantum <=0){ //if the quantum has expired
        ...
        // ── Selection ───────────────────────────────────
        // RR is quantum based, essentially pre-emptive. 
        // It selects whatever process is at the head of the queue after each quantum is complete.
        if (p == NULL) {; 
            // Lock readyQ before inspecting or modifying it — another CPU
            // thread (or main inserting a new arrival) could touch it right now.
            pthread_mutex_lock(&(svars->readyQLock));
...
void* SRTFcpu(void* param) {
    ...
    while (1) {
        ...
        if (p!=NULL){ //if there is a process currently running
            ...
            int shortestBurst = qShortestBR(&(svars->readyQ));//find the value of the shortest burst value in the ready queue
